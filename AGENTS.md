...
| `wiki/`     | Codex | **读写**    | 知识库。`index.md` 全局索引、`log.md` 操作日志、`overview.md` 顶层综述、`entities/`、`concepts/`、`sources/`、`comparisons/`、`analyses/`     |
| `AGENTS.md` | 共同     | 共同演进      | 本配置                                                                                                                    |
...
| Token  | `MINERU_TOKEN` 已在 `~/.Codex/settings.json` `env` 节；**不要**传 `--token`（会触发安全 hook）                                                      |
...
**触发**：Ingest 完成、Lint 修复、Query 归档、AGENTS.md 更新后**强制**同步。

**命令**：

```bash
git add wiki/ AGENTS.md raw/sources/
