---
title: "NVIDIA/Isaac-GR00T: NVIDIA Isaac GR00T N1.7 -  A Foundation Model for Generalist Robots."
source: "https://github.com/NVIDIA/Isaac-GR00T"
author:
published:
created: 2026-08-08
description: "NVIDIA Isaac GR00T N1.7 -  A Foundation Model for Generalist Robots. - NVIDIA/Isaac-GR00T"
tags:
  - "clippings"
---
[![NVIDIA Isaac GR00T N1.7 Header](https://github.com/NVIDIA/Isaac-GR00T/raw/main/media/header_compress.png)](https://github.com/NVIDIA/Isaac-GR00T/blob/main/media/header_compress.png)

[**Website**](https://developer.nvidia.com/isaac/gr00t) | [**Model**](https://huggingface.co/collections/nvidia/gr00t-n17) | [**Datasets (Physical AI)**](https://huggingface.co/collections/nvidia/physical-ai) | [**Paper**](https://arxiv.org/abs/2503.14734) | [**NVIDIA Isaac**](https://developer.nvidia.com/isaac) | [**FAQ**](https://github.com/NVIDIA/Isaac-GR00T/blob/main/FAQ.md)

## Table of Contents

## NVIDIA Isaac GR00T

| [![](https://github.com/NVIDIA/Isaac-GR00T/raw/main/media/unitree_g1.gif)](https://github.com/NVIDIA/Isaac-GR00T/blob/main/media/unitree_g1.gif) | [![](https://github.com/NVIDIA/Isaac-GR00T/raw/main/media/agibot_g1.gif)](https://github.com/NVIDIA/Isaac-GR00T/blob/main/media/agibot_g1.gif) | [![](https://github.com/NVIDIA/Isaac-GR00T/raw/main/media/yam.gif)](https://github.com/NVIDIA/Isaac-GR00T/blob/main/media/yam.gif) |
| --- | --- | --- |
...
[![model-architecture](https://github.com/NVIDIA/Isaac-GR00T/raw/main/media/model-architecture.png)](https://github.com/NVIDIA/Isaac-GR00T/blob/main/media/model-architecture.png)
...
4. **Evaluate** — Validate with [open-loop evaluation](#open-loop-evaluation), then test in [simulation benchmarks](#benchmark-examples) or on real hardware via the [Policy API](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/policy.md).
5. **Deploy** — Connect `Gr00tPolicy` to your robot controller, optionally accelerated with [TensorRT](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md).

## What's New in GR00T N1.7

GR00T N1.7 builds on N1.6 with a new VLM backbone and code-level improvements.

1. **Relative EEF Action Space** — N1.7 adopts a relative end-effector action space shared across robot and human embodiments. Representing actions as deltas from the current pose (rather than absolute targets) improves generalization and is a key factor in the model's cross-embodiment performance. See [`getting_started/finetune_new_embodiment.md`](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/finetune_new_embodiment.md) for guidance on configuring relative EEF for your own robot.
...
These are the main code, model, training, evaluation, and deployment changes that distinguish the current N1.7 main branch from the N1.6 / 1D6 code path. Use the [`n1d6` branch](https://github.com/NVIDIA/Isaac-GR00T/tree/n1d6) when you need the N1.6 model package and runtime behavior.
...
**Fine-tuning:** 1 or more GPUs with 40 GB+ VRAM recommended. We recommend H100 or L40 nodes for optimal performance. Other hardware (e.g., A6000) works but may require longer training time. See the [Hardware Recommendation Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/hardware_recommendation.md) for detailed specs.

**CUDA / Python per platform:** dGPU on CUDA 12.8 with Python 3.12; Jetson Orin on CUDA 12.6 with Python 3.10; Jetson Thor and DGX Spark on CUDA 13.0 with Python 3.12. The per-platform install scripts and Dockerfiles live under `scripts/deployment/`; see the [Deployment & Inference Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md) for the full matrix.
...
> **GB300 (sm\_103) Users:** Triton 3.3.1 (pinned by PyTorch 2.7) does not support the GB300 GPU architecture (sm\_103). `torch.compile` will fail on GB300. Use PyTorch eager mode or TensorRT inference instead. Triton 3.5.1+ adds sm\_103 support but is not yet compatible with the pinned PyTorch version.
...
See the [Spark setup guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md#dgx-spark-setup) for Docker and bare metal details.
...
See the [Thor setup guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md#jetson-thor-setup) for Docker and bare metal details.
...
See the [Orin setup guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md#jetson-orin-setup) for Docker and bare metal details.

> ⚠️
> 
> **aarch64 users (Spark / Thor / Orin):** After running `install_deps.sh`, always activate the venv with `source .venv/bin/activate && source scripts/activate_<platform>.sh` (`activate_spark.sh`, `activate_thor.sh`, or `activate_orin.sh`) and run the example commands in this guide with **plain `python`** / `torchrun`, not `uv run python` / `uv run torchrun`. The latter will re-sync against the root `pyproject.toml` (which targets x86\_64 Python 3.12) and destroy the platform-specific environment. See the [Deployment & Inference Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md#platform-specific-setup) for per-platform Docker and bare-metal setup.

For a containerized setup that avoids system-level dependency conflicts, see our [Docker Setup Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/docker/README.md). The recommended container workflow is to start the image first, then clone or pull the repo inside the running container so your checkout uses the image's prebuilt dependency environment.
...
| --- | --- | --- | --- |
| [`nvidia/GR00T-N1.7-3B`](https://huggingface.co/nvidia/GR00T-N1.7-3B) | Base | See [pretrain tags](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/policy.md#--embodiment-tag) | Base model (3B params) — zero-shot inference on pretrain embodiments, or finetune for new tasks |
...
For the full list of pretrain and posttrain tags, see the [Policy API Guide — Embodiment Tags](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/policy.md#--embodiment-tag).
...
| `demo_data/cube_to_bowl_5_with_mask` | SO100 arm + per-frame masks | `NEW_EMBODIMENT` | [Mask-guided background suppression](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/mask-guided-background-suppression/README.md) example |
...
See the full [Data Preparation Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/data_preparation.md) for schema details and examples.

---

## Inference

> **Prefer an interactive walkthrough?** The [`getting_started/GR00T_inference.ipynb`](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/GR00T_inference.ipynb) notebook steps through loading the model and predicting actions from observations on a sample dataset.
...
For connecting to a real robot (e.g., DROID hardware), see [examples/DROID/README.md](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/DROID/README.md). For faster inference with TensorRT, see the [Deployment & Inference Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/deployment/README.md).

See the complete [Policy API Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/policy.md) for documentation on observation/action formats, batched inference, and troubleshooting.
...
| --- | --- | --- |
| LIBERO | `LIBERO_PANDA` | [examples/LIBERO/README.md](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/LIBERO/README.md) |
| SimplerEnv (Fractal) | `SIMPLER_ENV_GOOGLE` | [examples/SimplerEnv/README.md](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/SimplerEnv/README.md) |
| SimplerEnv (Bridge) | `SIMPLER_ENV_WIDOWX` | [examples/SimplerEnv/README.md](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/SimplerEnv/README.md) |
| SO100 | `NEW_EMBODIMENT` | [examples/SO100/README.md](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/SO100/README.md) |
...
### Fine-tune on Your Own Robot ("NEW\_EMBODIMENT")

To finetune GR00T on your own robot data and configuration, follow the detailed tutorial at [`getting_started/finetune_new_embodiment.md`](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/finetune_new_embodiment.md).
...
Replace `demo_data/cube_to_bowl_5` and `examples/SO100/so100_config.py` with your own dataset and modality config. See [`examples/SO100`](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/SO100/README.md) for a complete walkthrough.
...
See the complete [Policy API Guide](https://github.com/NVIDIA/Isaac-GR00T/blob/main/getting_started/policy.md) for observation/action formats, batched inference, and troubleshooting.
...
You can use [the verification script](https://github.com/NVIDIA/Isaac-GR00T/blob/main/scripts/eval/check_sim_eval_ready.py) to verify that all dependencies are properly configured.
...
- [DROID](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/DROID/README.md) — real-world DROID robot (also available as the finetuned `nvidia/GR00T-N1.7-DROID` checkpoint; `examples/DROID/README.md` covers both paths)

**Finetuned** (evaluate with finetuned checkpoints):

- [DROID](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/DROID/README.md) — real-world DROID robot via `nvidia/GR00T-N1.7-DROID`
- [RoboLab](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/RoboLab/README.md) — RoboLab simulation tasks via `nvidia/GR00T-N1.7-DROID`
- [LIBERO](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/LIBERO/README.md) — LIBERO benchmark (Franka Panda)
- [SimplerEnv](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/SimplerEnv/README.md) — Google Robot (Fractal) and WidowX (Bridge)
- [SO100](https://github.com/NVIDIA/Isaac-GR00T/blob/main/examples/SO100/README.md) — SO100 custom embodiment workflow
**Adding a New Sim Benchmark**

Each sim benchmark registers its environments under a gym env\_name with the format `{prefix}/{task_name}` (e.g., `libero_sim/LIVING_ROOM_SCENE2_put_soup_in_basket`). The evaluation framework uses the prefix to look up the corresponding `EmbodimentTag` via a mapping in [`gr00t/eval/sim/env_utils.py`](https://github.com/NVIDIA/Isaac-GR00T/blob/main/gr00t/eval/sim/env_utils.py).

> **Important:** The env\_name prefix and the `EmbodimentTag` **name** are often different. For example, the prefix `libero_sim` maps to `EmbodimentTag.LIBERO_PANDA` (whose value happens to be `"libero_sim"`). Do not assume the prefix matches the tag name.
...
1. Add an entry to `ENV_PREFIX_TO_EMBODIMENT_TAG` in `gr00t/eval/sim/env_utils.py`:
	...
	```
2. If the benchmark has multiple env\_name prefixes (e.g., `my_benchmark_v1`, `my_benchmark_v2`), all related prefixes **must** map to the same `EmbodimentTag`.
...
We welcome issues and pull requests. See [CONTRIBUTING.md](https://github.com/NVIDIA/Isaac-GR00T/blob/main/CONTRIBUTING.md) for how to contribute and for support details now that GR00T N1.7 has reached General Availability (GA).

## License

- **Code:** Apache 2.0 — see [LICENSE](https://github.com/NVIDIA/Isaac-GR00T/blob/main/LICENSE)
...
@inproceedings{gr00tn1_2025,
  ...
  title      = {{GR00T} {N1}: An Open Foundation Model for Generalist Humanoid Robots},
       = {NVIDIA and Johan Bjorck and Fernando Castañeda, Nikita Cherniadev and Xingye Da and Runyu Ding and Linxi "Jim" Fan and Yu Fang and Dieter Fox and Fengyuan Hu and Spencer Huang and Joel Jang and Zhenyu Jiang and Jan Kautz and Kaushil Kundalia and Lawrence Lao and Zhiqi Li and Zongyu Lin and Kevin Lin and Guilin Liu and Edith Llontop and Loic Magne and Ajay Mandlekar and Avnish Narayan and Soroush Nasiriany and Scott Reed and You Liang Tan and Guanzhi Wang and Zu Wang and Jing Wang and Qi Wang and Jiannan Xiang and Yuqi Xie and Yinzhen Xu and Zhenjia Xu and Seonghyeon Ye and Zhiding Yu and Ao Zhang and Hao Zhang and Yizhou Zhao and Ruijie Zheng and Yuke Zhu},
  month      = {March},
  year       = {2025},
