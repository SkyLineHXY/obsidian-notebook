...
![](images/8f2fcb96093d07db75c1b6d80003037848ea4418468ee2471642dea1c07323cf.jpg)  
...
$\begin{array} { r } { \rho _ { \pi } ( \tau ) = p ( \mathbf { o } _ { 0 } ) \prod _ { t = 0 } ^ { T - 1 } \pi ( \mathbf { a } _ { t } | \mathbf { o } _ { t } ) p ( \mathbf { o } _ { t + 1 } | \mathbf { o } _ { t } , \mathbf { a } _ { t } ) } \end{array}$ .1 The reward function is given by $r ( \mathbf { o } _ { t } , \mathbf { a } _ { t } )$ , and we abbreviate it to $r _ { t }$ to shorten notation, where $r _ { T }$ is the terminal reward. We can define the discounted cumulative reward, or return, as $\begin{array} { r } { R ( \tau ) = \sum _ { t = 0 } ^ { T } r _ { t } } \end{array}$ (we do not use a discount factor, though one could easily be added). The goal of RL is to maximize the cumulative reward (or return), learning a policy that maximizes $\begin{array} { r } { \mathcal { I } ( \pi ) = \mathbb { E } _ { \tau \sim \rho _ { \pi } } [ R ( \tau ) ] = \mathbb { E } _ { \tau \sim \rho _ { \pi } } [ \sum _ { t = 0 } ^ { T } r _ { t } ] } \end{array}$ . The value function for a policy $\pi$ is then defined as $\begin{array} { r } { \overline { { V ^ { \pi } ( \mathbf { o } _ { t } ) } } = \mathbb { E } _ { \tau _ { t + 1 : T } } [ \sum _ { t = t } ^ { T } r _ { t } ] } \end{array}$ . We can then calculate an advantage value for an action $\mathbf { a } _ { t }$ as $\begin{array} { r } { A ^ { \pi } ( \mathbf { o } _ { t } , \mathbf { a } _ { t } ) \ = \ \mathbb { E } _ { \rho _ { \pi } ( \tau ) } [ \sum _ { t ^ { \prime } = t } ^ { t + N - 1 } \tilde { \mathbf { \Gamma } } _ { t ^ { \prime } } ^ { - } + V ^ { \pi } ( \mathbf { o } _ { t + N } ) ] \ - \ V ^ { \pi } ( \mathbf { o } _ { t } ) , } \end{array}$ , corresponding to an n-step estimate.

Regularized reinforcement learning. Instead of maximizing $\mathcal { I } ( \pi )$ , it is common to use regularization in RL, optimizing for a policy that maximizes reward while remaining close to some reference policy $\pi _ { \mathrm { r e f } }$ [66–70]. This is important, for example, when we want to train for many gradient steps on the same data, in which case $\pi _ { \mathrm { r e f } }$ typically corresponds to the behavior policy that collected the training data. This can be formalized via the objective $\mathcal { I } ( \pi , \pi _ { \mathrm { r e f } } ) =$ Eτ∼ρπ $\begin{array} { r } { \mathbb { E } _ { \tau \sim \rho _ { \pi _ { \theta } } } [ \sum _ { t = 0 } ^ { T } \gamma ^ { t } r _ { t } ] \ - \ \beta \mathbb { E } _ { \mathbf { o } \sim \rho _ { \pi _ { \theta } } } [ D ( \pi ( \cdot | \mathbf { \tilde { o } } ) | | \pi _ { \mathrm { r e f } } ( \cdot | \mathbf { o } ) ) ] } \end{array}$ , where $D$ denotes some divergence metric. For the case where $D$ is the KL divergence, we have the well-known result that $\hat { \pi } ( \mathbf { a } | \mathbf { o } ) \propto \pi _ { \mathrm { r e f } } ( \mathbf { a } | \mathbf { o } ) \exp ( A ^ { \pi _ { \mathrm { r e f } } } ( \mathbf { o } , \mathbf { a } ) / \beta )$ is the solution to $\operatorname* { m a x } _ { \pi } J ( \pi , \pi _ { \mathrm { r e f } } )$ , with Lagrange multiplier $\beta$ [67–70]. Our advantage-conditioned policy extraction method is based on a closely related but less well-known result: if we define the policy $\hat { \pi } ( \mathbf { a } | \mathbf { o } ) \propto { \pi } _ { \mathrm { r e f } } ( \mathbf { a } | \mathbf { o } ) p ( I | A ^ { \pi _ { \mathrm { r e f } } } ( \mathbf { o } , \mathbf { a } ) ) ^ { \beta }$ , where $\begin{array} { r } { p ( I \vert { \cal A } ^ { \pi _ { \mathrm { r e f } } } ( { \bf o } , { \bf a } ) ) = { g } ( { \cal A } ^ { \pi _ { \mathrm { r e f } } } ( { \bf o } , { \bf a } ) ) / \int { g } ( { \cal A } ^ { \pi _ { \mathrm { r e f } } } ( { \bf o } , { \bf a } ^ { \prime } ) ) \mathrm { d } { \bf a } ^ { \prime } } \end{array}$ is the probability of any action a improving over $\pi _ { \mathrm { r e f } }$ as measured by a monotonically increasing function $g$ , then $\hat { \pi }$ is guaranteed to improve over $\pi _ { \mathrm { r e f } }$ , i.e., $\mathcal { I } ( \hat { \pi } ) \geq \mathcal { I } ( \pi _ { \mathrm { r e f } } )$ [4, 71]. We will use this property in deriving our policy extraction method in Section IV-B. Using this definition we can then obtain a parametric policy from the closed form definition of $\hat { \pi }$ by solving the following minimization problem: $\begin{array} { r } { \operatorname* { m i n } _ { \theta } \mathbb { E } _ { s \sim \rho _ { \pi _ { \mathrm { r e f } } } } [ K L ( \hat { \pi } , \pi _ { \theta } ) ] } \end{array}$ .
...
$$
\min  _ {\phi} \mathbb {E} _ {\tau \in \mathcal {D}} \left[ \sum_ {\mathbf {o} _ {t} \in \tau} H \left(R _ {t} ^ {B} (\tau), p _ {\phi} (V | \mathbf {o} _ {t}, \ell)\right) \right]. \tag {1}
$$
...
Once we have the value function $V ^ { \pi _ { \mathrm { r e f } } }$ , we need a way to train an improved policy using this value function. This is called policy extraction. An effective policy extraction method in our setting needs to satisfy several criteria. First,

![](images/6922ccdcbd12b306aa250949e2db4d1b1abdc2974a6478829325dd505af935ca.jpg)  
...
![](images/70c1cb82475e8152572b1e13f879c6c47e51ae13a029dc4f65a444bb02fb527c.jpg)

![](images/03c8bbab1e9220b8e356e2a06d40b3de69f6fe23a704a91d1bcaab08af9716c9.jpg)  
...
$$
\hat {\pi} (\mathbf {a}, | \mathbf {o}, \ell) \propto \pi_ {\mathrm {r e f}} (\mathbf {a} | \mathbf {o}, \ell) \left(\frac {\pi_ {\mathrm {r e f}} (\mathbf {a} | I , \mathbf {o} , \ell)}{\pi_ {\mathrm {r e f}} (\mathbf {a} | \mathbf {o} , \ell)}\right) ^ {\beta}. \tag {2}
$$
...
$$
p (I \mid A ^ {\pi_ {\text {r e f}}} (o, a, \ell)) = \delta \left(A ^ {\pi_ {\text {r e f}}} (o, a, \ell) > \epsilon_ {\ell}\right),
$$
...
$$
\min  _ {\theta} \mathbb {E} _ {\mathcal {D} _ {\pi_ {\text {r e f}}}} \left[ - \log \pi_ {\theta} \left(\mathbf {a} _ {t} \mid \mathbf {o} _ {t}, \ell\right) - \alpha \log \pi_ {\theta} \left(\mathbf {a} _ {t} \mid I _ {t}, \mathbf {o} _ {t}, \ell\right) \right], \tag {3}
$$

where $I _ { t } = \mathbb { 1 } \left( A ^ { \pi _ { \mathrm { r e f } } } ( \mathbf { o } _ { t } , \mathbf { a } _ { t } , \ell ) > \epsilon _ { \ell } \right) .$

The advantage values $A ^ { \pi _ { \mathrm { r e f } } } ( \mathbf { o } _ { t } , \mathbf { a } _ { t } , \ell )$ are obtained from the value function in the previous section, and $\alpha$ is a tradeoff hyperparameter. In practice, the dataset $\mathcal { D } _ { \pi _ { \mathrm { r e f } } }$ consists of all of the data collected so far, including all demonstrations and autonomous task attempts, and the reference policy $\pi _ { \mathrm { r e f } }$ is therefore a mixture of human behavior and previously

2Prior work [4] instead uniformly chose $\epsilon = 0$ and tuned $\beta$ at test time, as in classifier-free guidance (CFG). However, high CFG weights can drive the action distribution to the corners of its support (leading to aggressive behavior) and would not affect the autoregressive part of the model. We found it easier to obtain good results by instead using the threshold $\epsilon _ { \ell }$ to trade off regularization and optimality.

deployed policies. To include human corrections, we found it useful to force $I _ { t } =$ True (i.e., positive) for actions provided as human corrections during autonomous rollouts. This choice is reasonable if we assume that human experts always provide good corrective actions. As we will discuss in Section V, in practice our VLA model produces both discrete and continuous outputs, with the continuous distribution represented via flow matching. Therefore, the real training objective combines likelihoods for the discrete values with the flow matching objective for the continuous values.
...
We instantiate RECAP with a VLA that we call $\pi _ { 0 . 6 } ^ { * }$ . π ∗0.6 is based on the $\pi _ { 0 . 6 }$ VLA, which is an evolution of the $\pi _ { 0 . 5 }$ VLA [5] with a few improvements that we detail in the accompanying model card [6]. $\pi _ { 0 . 6 } ^ { * }$ additionally adds the ability condition on the binarized advantage indicator $I _ { t }$ , making it suitable for RL training with RECAP. The model architecture is illustrated in Figure 3. We train a value function alongside

Algorithm 1 RL with Experience and Corrections via Advantage-conditioned Policies (RECAP)   
Require: multi-task demonstration dataset $\mathcal{D}_{\mathrm{demo}}$ 1: Train $V_{\mathrm{pre}}$ on $\mathcal{D}_{\mathrm{demo}}$ using Eq. 1 2: Train $\pi_{\mathrm{pre}}$ on $\mathcal{D}_{\mathrm{demo}}$ using Eq. 3 and $V_{\mathrm{pre}}$ 3: Initialize $\mathcal{D}_{\ell}$ with demonstrations for $\ell$ 4: Train $V_{\ell}^{0}$ from $V_{\mathrm{pre}}$ on $\mathcal{D}_{\ell}$ using Eq. 1 5: Train $\pi_{\ell}^{0}$ from $\pi_{\mathrm{pre}}$ on $\mathcal{D}_{\ell}$ using Eq. 3 and $V_{\ell}^{0}$ 6: for $k = 1$ to $K$ do 7: Collect data with $\pi_{\ell}^{k - 1}$ , add it to $\mathcal{D}_{\ell}$ 8: Train $V_{\ell}^{k}$ from $V_{\mathrm{pre}}$ on $\mathcal{D}_{\ell}$ using Eq. 1 9: Train $\pi_{\ell}^{k}$ from $\pi_{\mathrm{pre}}$ on $\mathcal{D}_{\ell}$ using Eq. 3 and $V_{\ell}^{k}$ 10: end for
...
$$
\begin{array}{l} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H}, a _ {t: t + H} ^ {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \geq \\ \mathbb {E} _ {\eta , \omega} \left[ \log p _ {\theta} \left(a _ {t: t + H} ^ {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) - \right. \tag {4} \\ \alpha_ {\eta} \left\| \omega - \mathbf {a} _ {t: t + H} - f _ {\theta} \left(\mathbf {a} _ {t: t + H} ^ {\eta , \omega}, I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \right\| ^ {2} ] \\ \end{array}
$$

with the n $\mathbf { a } _ { t : t + H } ^ { \eta , \omega } = \eta \mathbf { a } _ { t : t + H } + ( 1 - \eta ) \omega$ $\boldsymbol \omega \sim \mathcal { N } ( 0 , \bf { I } )$ denotinghing time $\eta \in [ 0 , 1 ]$ index and $f _ { \theta }$ denotes the continuous outputs of the diffusion expert. $\alpha _ { \eta }$ is a loss weighting term (which can optionally be noise dependent). Full details for the loss are provided in Appendix C.
...
$$
r _ {t} = \left\{ \begin{array}{l l} 0 & \text {i f t = T a n d s u c c e s s} \\ - C _ {\text {f a i l}} & \text {i f t = T a n d f a i l u r e} \\ - 1 & \text {o t h e r w i s e .} \end{array} \right. \tag {5}
$$
...
The data mixture used in the pre-training phase of our model largely follows the recipe used by $\pi _ { 0 . 5 }$ [5], with visionlanguage data from the web, prediction of subtasks $\hat { \ell } .$ , and prediction of low-level actions on a variety of tasks from many different robots. We note that, after pre-training, $\pi _ { 0 . 6 } ^ { * }$ can perform many more tasks than the ones used in evaluation in Section VI. During pre-training, we first train the value function on the same dataset, predicting (the negative of) the number of steps to successful completion of each task. Then we estimate the per-task improvement threshold, $\epsilon _ { \ell }$ , used in determining the advantage-based improvement indicator $I _ { t }$ . We set $\epsilon _ { \ell }$ to the $3 0 \%$ percentile of values predicted by the value function for the task ℓ. We then run the value function on-the-fly during VLA training to estimate $A ^ { \pi _ { \mathrm { r e f } } } ( \mathbf { o } _ { t } , \mathbf { a } _ { t } , \ell )$ for each example, and then use it to compute $I _ { t }$ based on ϵℓ. $I _ { t }$ is included as an input to $\pi _ { 0 . 6 } ^ { * }$ as described in Section V-A. As we use a relatively small VLM backbone (670M) for the value function, on-the-fly inference of the value function incurs minimal additional cost during VLA training.

After pre-training we start a policy improvement loop for the target task. We first finetune $\pi _ { 0 . 6 } ^ { * }$ with demonstration data $\mathcal { D } _ { \ell }$ for the target task ℓ. We fix the indicator $I _ { t }$ to True in this stage, which we found to lead to slightly better results, such that this stage corresponds to supervised finetuning (SFT). This results in the initial policy $\pi _ { \ell } ^ { 0 }$ , which is then used to collect additional data that is added to $\mathcal { D } _ { \ell }$ . While some of the episodes are collected fully autonomously, some are monitored by an expert teleoperator who can intervene to

![](images/b6ed692fea1aea29ff60104b2f1e073aeec4b9701f4fbbdf555b33693475cebe.jpg)  
...
![](images/571cd11e4c49b12f0eed53a2c6550ef49940ee6c5b20aafed50810d5cb763f82.jpg)

![](images/fd228b0158ac369bf83fed87e24aee3ee73a730af2b5ccdd136ca00194009d2f.jpg)

![](images/f15f67601cd533fd05489a5bec3459a8fb75946b088ad1d34f34783e5dd12932.jpg)

![](images/74394ca4f921310312523ae2ac6ce818235d7118dd03bc86b7361f353382f785.jpg)

![](images/9ccfbedf080970ff88766f7989a7d31030c17b5673520f47cdf7fa9ff9183a7e.jpg)

![](images/a0d68d67f174ccd85d0c559d4933f5f1700a91927e127c971da791f9dfaf277c.jpg)

![](images/525298045093cf4b9f707df195dd39fa2c7671c720c8f6409050f0957591b740.jpg)

![](images/25275bbfa24ccc4bf1ea7b61fe27efcf8cf58398e74704ba27318183fd9dc2ed.jpg)

![](images/20ce01b836eae2e0e665ca7a01180940c339e5dbb48385c21abf89a12cb409ba.jpg)

![](images/589667cdc4c919e3a86139c68a8d61442fe8db1db95135ecbd8ae55d7ea23f00.jpg)  
...
$\pi _ { 0 . 6 } ^ { * }$ offline $\mathbf { R L } + \mathbf { S F T }$ . This model is trained by finetuning the base $\pi _ { 0 . 6 } ^ { * }$ pre-trained checkpoint with demonstration data for the target task. We refer to this finetuning as “SFT” because the advantage values are fixed to True for all demonstrations. We find that this combination of the offline RL pre-trained $\pi _ { 0 . 6 } ^ { * }$ model with high-quality SFT outperforms standard SFT

![](images/4907d49e18b1a834d8de91eeae3460329a4dc51c096f5341de41c9e7a930483d.jpg)

![](images/e358914cc49e856584973ec3fbe72d30edc19880df626f621cc8917af9516ed0.jpg)

![](images/2a05af44f8e5e1a5ec36dab4a27803a73e56e207ddf7969c607000020aa55eea.jpg)

![](images/e964b5c29b628a3df9bec7f4e606dcc7b47b730b308e98874911ba0ee0948652.jpg)  
...
![](images/d76ae23f2069882899a7f01613f6b1d4ceaeb6e14a3723548b4d948e21e12982.jpg)

![](images/a5a860aeebabb68a1675072fa2e3749ae0c02c61f7da1710d25cf9734052c48b.jpg)

![](images/94e169c7aefacade6964c6ac9ec7e01a04c6a74f41d7464a63022723e720a4d1.jpg)

![](images/20dcc759744bb4ad31cb2a8d23288bb9375069427d62db7b3402bae3a8b23ad0.jpg)  
...
![](images/b1bb74834b37eac88c58c48d8e25124aa53c4c3b0d68689d938459b8f4401f5a.jpg)  
...
![](images/25a611009d00fd94e2e0a874c502a70dfb366f2fbf3707e4fffa58ef36e126b2.jpg)  
...
![](images/3ad1f1705249c17bfeade432f096c1c53147fb8235bbec07bba6ba83999cc350.jpg)  
...
![](images/9fdd0384ce907447abc46f8f65391f676da693f74d94faa999a4895172089d77.jpg)  
...
[7] Stephane Ross, Geoffrey Gordon, and Drew Bagnell. A ´ reduction of imitation learning and structured prediction to no-regret online learning. In AISTATS, pages 627–635, 2011. 2, 7   
...
[18] Russell Mendonca, Emmanuel Panov, Bernadette Bucher, Jiuguang Wang, and Deepak Pathak. Continuously improving mobile manipulation with autonomous realworld rl. In Proceedings of the 8th Conference on Robot Learning (CoRL), pages 5204–5219, 2024.   
...
[26] Abhishek Gupta, Justin Yu, Tony Z. Zhao, Vikash Kumar, Aaron Rovinsky, Kelvin Xu, Thomas Devlin, and Sergey Levine. Reset-free reinforcement learning via multitask learning: Learning dexterous manipulation behaviors without human intervention. In Proceedings of the 2021 IEEE International Conference on Robotics and Automation (ICRA), pages 6664–6671, 2021. 2   
...
[30] Shuhan Tan, Kairan Dou, Yue Zhao, and Philipp Krahenb ¨ uhl.¨ Interactive post-training for vision-language-action models. arXiv preprint, arXiv:2505.17016, 2025. 2   
...
Yunfei Ge, Zhenglong Sun, Xiu Li, Chi Zhang, Chenjia Bai, and Xuelong Li. Align-then-steer: Adapting the vision-language action models through unified latent guidance. arXiv preprint arXiv:2509.02055, 2025. 2   
...
[47] Jurgen Schmidhuber. Reinforcement learning upside ¨ down: Don’t predict rewards — just map them to actions. arXiv preprint, arXiv:1912.02875, 2019. 3, 4   
...
[53] Taku Yamagata, Ahmed Khalil, and Raul Santos- ´ Rodr´ıguez. Q-learning decision transformer: Leveraging dynamic programming for conditional sequence modelling in offline rl. In Proceedings of the 40th International Conference on Machine Learning (ICML), volume 202 of Proceedings of Machine Learning Research, pages 38989–39007. PMLR, 2023.   
...
[60] Sumedh A. Sontakke, Jesse Zhang, Sebastien M.R.´ Arnold, Karl Pertsch, Erdem Bıyık, Dorsa Sadigh, Chelsea Finn, and Laurent Itti. Roboclip: One demonstration is enough to learn robot policies. In Proceedings of the 37th Conference on Neural Information Processing Systems (NeurIPS), 2023.   
...
Sontakke, Joseph J. Lim, Jesse Thomason, Erdem Bıyık, and Jesse Zhang. Rewind: Language-guided rewards teach robot policies without new demonstrations. In Proceedings of the 9th Conference on Robot Learning (CoRL), 2025.   
...
[69] Peter Dayan and Geoffrey E. Hinton. Using expectationmaximization for reinforcement learning. Neural Computation, 9(2):271–278, 1997. doi: 10.1162/neco.1997.9. 2.271.   
[70] Jan Peters, Katharina Mulling, and Yasemin Alt¨ un. Rel-¨ ative entropy policy search. In Proceedings of the Twenty-Fourth AAAI Conference on Artificial Intelligence, AAAI’10, page 1607–1612. AAAI Press, 2010. 3   
...
[72] Marc G Bellemare, Will Dabney, and Remi Munos. ´ A distributional perspective on reinforcement learning. In International conference on machine learning, pages
...
Brian Ichter, Liyiming Ke, Sergey Levine, Suraj Nair, Allen Z. Ren, Laura Smith, Jost Tobias Springenberg, Zhiyuan Zhou B. Additional Value Function Visualization
...
$$
\begin{array}{l} \pi_ {\theta} \left(\mathbf {a} _ {t: t + H}, \mathbf {a} _ {t: t + H} ^ {\ell}, \hat {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell\right) = \\ \pi_ {\theta} \left(\mathbf {a} _ {t: t + H} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \pi_ {\theta} \left(\mathbf {a} _ {t: t + H} ^ {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \pi_ {\theta} (\hat {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell), \tag {6} \\ \end{array}
$$
...
$$
\begin{array}{l} \log \pi_ {\theta} (\mathbf {a} _ {t: t + H} | \mathbf {a} _ {1: H} ^ {\eta , \omega}, I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}) = \\ \log \mathcal {N} \left(\omega - f _ {\theta} \left(\mathbf {a} _ {1: H} ^ {\eta , \omega}, I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right), \mathbf {I}\right), \tag {7} \\ \end{array}
$$
...
$$
\begin{array}{l} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \geq \\ \frac {1}{2} \mathbb {E} _ {\eta , \omega} \left[ - w (\eta) \left\| \omega - \mathbf {a} _ {1: H} - f _ {\theta} \left(\mathbf {a} _ {1: H} ^ {\eta , \omega}, I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \right\| ^ {2} \right] + c, \tag {8} \\ \end{array}
$$
...
$$
\begin{array}{l} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H}, \mathbf {a} _ {t: t + H} ^ {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \geq \\ \mathbb {E} _ {\eta , \omega} \left[ \log p _ {\theta} \left(\mathbf {a} _ {t: t + H} ^ {\ell} \mid I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \right. \tag {9} \\ \left. - \alpha_ {\eta} \left\| \omega - \mathbf {a} _ {1: H} - f _ {\theta} \left(\mathbf {a} _ {1: H} ^ {\eta , \omega}, I _ {t}, \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \right\| ^ {2} \right], \\ \end{array}
$$

which is the bound given in the main part of the paper.

![](images/e5001c78e1bcb3eef6e813656bedc58e1b958fa71d1088a179780258f6eb4f70.jpg)

![](images/5ad4f69781765b121e4c27eec0f953bd1292c1b879bd7f7c6a9444a5d2234605.jpg)

![](images/07d9a4bd463f95947c7a9d2a467bd5cbe0e53de90a64bc1df56eff344e0c7f8d.jpg)

![](images/fb03f0317a116d80f6f9d0054dfecafc59c6d7188c234b202272573bbdbeda1e.jpg)

![](images/394fd1fa07ad27da1e9d6d15b3e03c29cd2f4956ef6320289ea1d6780b9838ac.jpg)

![](images/fc4faebe63837c752f8c5c3d88b02f519ede20cf1c8cdaca458ec3091bb9513a.jpg)

![](images/d232b113db43f3a70b4f4b3932db2dc7314b02d33daa92a7780c2eda5941ffde.jpg)  
...
$$
\begin{array}{l} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H}, \mathbf {a} _ {t: t + H} ^ {\ell} \mid \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \geq \\ \mathbb {E} _ {\eta , \omega} \left[ \log p _ {\theta} \left(\mathbf {a} _ {t: t + H} ^ {\ell} \mid \mathbf {o} _ {t}, \ell , \hat {\ell}\right) \right. \tag {10} \\ \left. - \alpha_ {\eta} \left\| \boldsymbol {\omega} - \mathbf {a} _ {1: H} - f _ {\theta} (\mathbf {a} _ {1: H} ^ {\eta , \omega}, \mathbf {o} _ {t}, \ell , \hat {\ell}) \right\| ^ {2} \right], \\ \end{array}
$$
...
$$
\begin{array}{l} \mathcal {L} _ {S P O + C o V L A} (\theta) = \\ \left\{ \begin{array}{l} \frac {\pi_ {\theta} (a _ {\hat {\ell}} \in \hat {\ell} | \mathbf {o} _ {t} , \ell)}{\pi_ {\mathrm {r e f}} (a _ {\hat {\ell}} \in \hat {\ell} | \mathbf {o} _ {t} , \ell)} A ^ {\pi_ {\mathrm {r e f}}} (o _ {t}, a _ {t}, \ell) \end{array} \right. \\ \left. - \frac {\left| A ^ {\pi_ {\mathrm {r e f}}} \left(o _ {t} , a _ {t} , \ell\right) \right|}{2 \epsilon_ {\mathrm {a r}}} \left[ \frac {\pi_ {\theta} \left(a _ {\hat {\ell}} \in \hat {\ell} \mid \mathbf {o} _ {t} , \ell\right)}{\pi_ {\mathrm {r e f}} \left(a _ {\hat {\ell}} \in \hat {\ell} \mid \mathbf {o} _ {t} , \ell\right)} - 1 \right] \right\} \tag {11} \\ + \alpha \left\{\frac {\pi_ {\theta} (\mathbf {a} _ {t : t + H} | \mathbf {o} _ {t} , \ell)}{\pi_ {\mathrm {r e f}} (\mathbf {a} _ {t : t + H} | \mathbf {o} _ {t} , \ell)} A ^ {\pi_ {\mathrm {r e f}}} (o _ {t}, a _ {t}, \ell) \right. \\ \left. - \frac {| A ^ {\pi_ {\mathrm {r e f}}} (o _ {t} , a _ {t} , \ell) |}{2 \epsilon_ {\mathrm {f l o w}}} \left[ \frac {\pi_ {\theta} (\mathbf {a} _ {t : t + H} | \mathbf {o} _ {t} , \ell)}{\pi_ {\mathrm {r e f}} (\mathbf {a} _ {t : t + H} | \mathbf {o} _ {t} , \ell)} - 1 \right] \right\}, \\ \end{array}
$$

where $\alpha$ is a trade-off parameter and $\epsilon _ { \mathrm { a r } }$ , ϵflow are trust-region parameters for autoregressive and flow-matching model parts respectively. We use this variant to perform training on eval data starting from the $\pi _ { 0 . 6 }$ checkpoint.
...
$$
\hat {\pi} \left(\mathbf {a} _ {t: t + H} \mid \mathbf {o} _ {t}, \ell\right) \propto \pi_ {\text {r e f}} \left(\mathbf {a} _ {t: t + H} \mid \mathbf {o} _ {t}, \ell\right) \left(\frac {\pi_ {\text {r e f}} \left(\mathbf {a} _ {t : t + H} \mid I _ {t} , \mathbf {o} _ {t} , \ell\right)}{\pi_ {\text {r e f}} \left(\mathbf {a} _ {t : t + H} \mid \mathbf {o} _ {t} , \ell\right)}\right) ^ {\beta}. \tag {12}
$$
...
$$
\begin{array}{l} \nabla_ {\mathbf {a}} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H} | \mathbf {o} _ {t}, \ell\right) + \\ \beta \left(\nabla_ {\mathbf {a}} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H} \mid I _ {t}, \mathbf {o} _ {t}, \ell\right) - \nabla_ {\mathbf {a}} \log \pi_ {\theta} \left(\mathbf {a} _ {t: t + H} \mid \mathbf {o} _ {t}, \ell\right)\right), \tag {13} \\ \end{array}
$$

we are effectively sampling from the desired attenuated distribution. We note that, as mentioned in the main paper, the parameter $\beta$ is loosely connected to the advantage threshold $\epsilon _ { \ell }$ that we introduce during training (in the sense that both sharpen the distribution, one at inference and one at training time). We find that sharpening the distribution after training with high settings for $\beta$ can lead to pushing the action distribution towards the boundaries of its learned support (which can lead to overly aggressive motions) and thus primarily rely on $\epsilon _ { \ell }$ for obtaining a good conditioned policy directly after training and combine it with moderate settings (e.g. $\beta \in [ 1 . 5 , 2 . 5 ] ,$ $\beta \in$ ) where useful.
